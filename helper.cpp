...
void humanLeft(State& state){
	float pos = state.left.getPos();
	float offset = state.left.getOffset();
	float new_pos = max(-1.f+PADDLE_HEIGHT, min(1.f-PADDLE_HEIGHT, pos + state.paddle_speed*state.left.getVel()));
	state.left.setPos(new_pos, offset);
	//state.left.setVel(0.f); //Allows continuous movement
}
...
void update(int value) {
	...
	*/
	...
	if(double(clock()-state.begin)/CLOCKS_PER_SEC > 20){
		cout << "time out" << endl;
		state.reset();
		store.clear();
		glutLeaveMainLoop();
